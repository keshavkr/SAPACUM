 ...
 *
 */
public class SAPACUMTestUtil {	
	
	private static final Log LOGGER = Log.getLog(SAPACUMTestUtil.class);
	...
	public SAPACUMConfiguration newacConfiguration() {
		...
		PropertyBag properties = TestHelpers.getProperties(SAPACUMConnector.class);	
		...
		try {
			for(Method method : methods){
			   ...
			   if(paramType[0].equals(int.class)){				   
				   ...
				   if (property.equalsIgnoreCase("password")||property.equalsIgnoreCase("dummypassword")|| property.equalsIgnoreCase("grcPassword")){					   
	...
	private Map<String, Object> toMap(int dataCase, List<String> attrToGet) {
		...
		try {
			if (prop == null){
				prop = TestHelpers.getProperties(SAPACUMConnector.class);
			}
     ...
     */
     public Attribute getChildTableAttr(int dataCase, List<String> attrs, String childTableName) {
            ...
            try {
                   ...
                   if(SAPACUMTestUtil.ROLE.equals(childTableName)){
                         childDataObjClassName="ACTIVITYGROUPS";
                         childDataKeyName ="roles" ;
                   } else if(SAPACUMTestUtil.PROFILE.equals(childTableName)){
                         childDataObjClassName="PROFILES";
                         childDataKeyName = "profiles";
                   } else if(SAPACUMTestUtil.PARAMETER.equals(childTableName)){
                         childDataObjClassName="PARAMETER1";
                         childDataKeyName = "parameters";
                   } else if(SAPACUMTestUtil.GROUP.equals(childTableName)){
	...
	public String getReconQuery(int dataCase){
		...
		try{
			PropertyBag prop = TestHelpers.getProperties(SAPACUMConnector.class);
